...
class app():
    ...
    def Popular_Songs(self):
        ...
        img_tags = soup.find_all('img')
        urls = [img['src'] for img in img_tags] #Grabs the album cover's image source link
        self.Download(urls, 10)
        self.frame.pack_forget()
        self.frame = tk.Frame(self.root, bg= 'white')
        self.frame.pack()
        return_frame = tk.Frame(self.frame, bg = 'white')
        return_frame.pack(side=tk.BOTTOM)
        tk.Label(self.frame, text = 'Latest Popular Songs', bg = 'white', font = ("", int(self.height3))).pack(pady = self.height4 / 2)
        width, height =self.root.winfo_screenwidth(), int(self.full_Height/1.4)                 
        scroll_frame = tk.Frame(self.frame, bg = 'white') #Creating another scrollable frame
        scroll_frame.pack(side=tk.LEFT)    
        ...
        canvas_frame.bind("<Configure>", lambda event, canvas=Canvas:self.MyScrollControl(Canvas, width, height))
        counter = 1
        images = []
        for x in range(1, 11): #Converting the cover art to .gifs
            images.append(imageio.imread("temp" + str(x) + ".jpg"))
            imageio.mimsave("temp" + str(x) + '.gif', images)
            os.remove("temp" + str(x) + ".jpg")
            images = []
        ...
        for x in Songs:
            if counter <= 3:
                tk.Label(canvas_frame, text = "♚", bg = 'white', fg = colour[counter], font = ("", int(self.height4 / 1.5), "bold")).grid(row = counter, column = 0)
